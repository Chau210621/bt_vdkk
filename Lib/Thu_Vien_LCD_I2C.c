...
void lcd_send_cmd(unsigned char cmd)
{
	...
	cmd_l=(cmd<<4)&0xf0; // dich 4 bit thap len 
	cmd_u=(cmd &0xf0); // 4 bit cao
	
	i2c_start();			 //BL EN RW RS 1 1 0 0
	i2c_write(slave_add);
	i2c_ACK();
	i2c_write(cmd_u|0x0C);// xxxx 0000 | 0000 1100 = xxxx 1100
	i2c_ACK();
	delay_ms(1);
	i2c_write(cmd_u|0x08); // xxxx 0000 | 0000 1000 = xxxx 1000
	i2c_ACK();
	delay_ms(1);
